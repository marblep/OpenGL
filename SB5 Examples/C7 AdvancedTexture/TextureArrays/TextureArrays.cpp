...
void SetupRC()
    ...
    starFieldShader = gltLoadShaderPairWithAttributes("TextureArrays\\StarField.vp", "TextureArrays\\StarField.fp", 1, GLT_ATTRIBUTE_VERTEX, "vVertex");

	locMVP = glGetUniformLocation(starFieldShader, "mvpMatrix");
	locStarTexture = glGetUniformLocation(starFieldShader, "starImage");

	moonShader = gltLoadShaderPairWithAttributes("TextureArrays\\MoonShader.vp", "TextureArrays\\MoonShader.fp", 2, GLT_ATTRIBUTE_VERTEX, "vVertex",
	...
	glBindTexture(GL_TEXTURE_2D, starTexture);
	LoadTGATexture("TextureArrays\\star.tga", GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR, GL_CLAMP_TO_EDGE);
	...
	for(int i = 0; i < 29; i++) {
		char cFile[32];
		sprintf(cFile, "TextureArrays\\moon%02d.tga", i);
